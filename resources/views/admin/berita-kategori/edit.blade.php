...
@section('subtitle', 'Ubah nama kategori berita.')

@section('content')
<div class="max-w-xl bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
    <form action="{{ route('admin.berita-kategori.update', $beritaKategori) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-2">
            ...
            <input 
                ...
                name="name" 
                id="name" 
                value="{{ old('name', $beritaKategori->name) }}"
